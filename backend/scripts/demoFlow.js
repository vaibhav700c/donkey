 ...
 * demoFlow.js
 * 
 * Demonstrates the complete RSA-OAEP encryption workflow:
 * 1. Generate actor RSA keys (if missing)
 * 2. Encrypt sample file with AES-256-GCM
 * 3. Upload encrypted package to IPFS
 * 4. Wrap CEK for multiple actors using RSA-OAEP
 * 5. Demonstrate server-side unwrap (DEMO ONLY - not production)
 * 
 ...
 * Usage:
 *   npm run demo
 *   node scripts/demoFlow.js [--skip-ipfs] [--verbose]
 * 
 * Options:
...
const { generateAllKeys } = require('./genActorKeys');
...
/**
 * Get file paths for actor's public and private keys
 */
function keyPaths(actorId) {
  ...
  return {
    pub: path.join(KEY_DIR, `actor-${actorId}-${label}-pub.pem`),
    priv: path.join(KEY_DIR, `actor-${actorId}-${label}-priv.pem`),
  };
}

/**
 * Check if all required actor keys exist, generate if missing
 */
function ensureKeys() {
  try {
    if (!fs.existsSync(KEY_DIR)) {
      log('📁 Keys directory not found, generating keys...', 'info');
      generateAllKeys();
      return;
    }
    ...
    if (missingActors.length > 0) {
      log(`⚠️  Missing keys for actors: ${missingActors.join(', ')}`, 'warn');
      log('🔑 Generating missing keys...', 'info');
      generateAllKeys();
    } else {
      log('✅ All actor keys found', 'verbose');
    }
  } catch (error) {
    throw new Error(`Failed to ensure keys: ${error.message}`);
  }
}

/**
 * Load RSA keys for all actors from filesystem
 */
function loadKeys() {
  try {
    log('📂 Loading actor RSA keys...', 'verbose');
    const keys = ACTOR_IDS.reduce((acc, actorId) => {
      const { pub, priv } = keyPaths(actorId);
      
      if (!fs.existsSync(pub) || !fs.existsSync(priv)) {
        throw new Error(`Missing keys for actor ${actorId} (${ACTOR_NAMES[actorId]})`);
      }
...
async function runDemoFlow() {
  ...
  console.log('\n🔐 RSA-OAEP Demo Flow - Cardano Healthcare Vault\n');
  ...
  try {
    ...
    // Step 1: Ensure actor keys exist
    log('\n📋 Step 1: Verify Actor Keys', 'info');
    ensureKeys();
    const actorKeys = loadKeys();
    logger.info('✅ Loaded RSA-4096 keys for all actors');
    ...
    const { package: encryptedPackage, iv, authTag } = cryptoService.encryptAESGCM(plainBuffer, cek);
    ...
    if (!SKIP_IPFS) {
      ...
      uploadResult = { cid: 'mock-cid-' + Date.now() };
      cidHash = cryptoService.sha256hex(uploadResult.cid);
      log('\n📋 Step 4: Upload to IPFS (SKIPPED)', 'info');
    }
    
    // Step 5: Wrap CEK for all actors
    log('\n📋 Step 5: Wrap CEK for Actors (RSA-OAEP)', 'info');
    const wrappedKeys = {};
    const wrapTimes = {};
    
    for (const actorId of ACTOR_IDS) {
      const wrapStart = Date.now();
      const wrapped = cryptoService.wrapKeyRSA(cek, actorKeys[actorId].publicKey);
      const wrapDuration = Date.now() - wrapStart;
      
      wrappedKeys[actorId] = wrapped.toString('base64');
      wrapTimes[actorId] = wrapDuration;
      
      logger.info(`✅ Actor ${actorId} (${ACTOR_NAMES[actorId]}): ${wrapped.length} bytes wrapped in ${wrapDuration}ms`);
      log(`   Base64 preview: ${wrappedKeys[actorId].substring(0, 40)}...`, 'verbose');
    }
    ...
    const doctorWrapped = Buffer.from(wrappedKeys[testActorId], 'base64');
    
    log(`   Unwrapping CEK for actor ${testActorId} (${ACTOR_NAMES[testActorId]})...`, 'verbose');
    const unwrapStart = Date.now();
    const unwrappedCEK = cryptoService.unwrapKeyRSA(doctorWrapped, actorKeys[testActorId].privateKey);
    ...
    console.log(`   CEK wrap (avg):        ${Math.round(Object.values(wrapTimes).reduce((a, b) => a + b, 0) / ACTOR_IDS.length)}ms`);
    ...
    console.log('   • Consider X25519 for better performance (see demoFlowX25519.js)');
    ...
    const result = {
      ...
      performanceMs: {
        total: totalDuration,
        wrapAvg: Math.round(Object.values(wrapTimes).reduce((a, b) => a + b, 0) / ACTOR_IDS.length),
    ...
    const totalDuration = Date.now() - startTime;
    console.log('\n━'.repeat(60));
    console.error(`\n❌ Demo Flow Failed after ${totalDuration}ms`);
...
if (require.main === module) {
  runDemoFlow()
    ...
    .catch((err) => {
      logger.error('❌ Demo flow failed:', err);
      process.exit(1);
    });
}
